...
deals = pd.read_csv('../data/deals.tsv',
                    sep='\t',
                    encoding='utf-8')
sectors = pd.read_csv('../data/sectors.tsv',
                      sep='\t',
                      encoding='utf-8')
companies = pd.read_csv('../data/companies.tsv',
                        sep='\t',
                        encoding='utf-8')
contacts = pd.read_csv('../data/contacts.tsv',
                       sep='\t',
                       encoding='utf-8')
...
deals['dealsDateCreated'] = pd. \
    to_datetime(deals['dealsDateCreated'])

contacts['contactsDateCreated'] = pd. \
    to_datetime(contacts['contactsDateCreated'])

companies['companiesDateCreated'] = pd. \
    to_datetime(companies['companiesDateCreated'])

# Valores inválidos
# Não é possível haver dealsDateCreated,
# contactsDateCreated e companiesDateCreated no futuro
# Então, me basendo na data que recebi o desafio
# '2018-10-09' para ser o fator seletivo

# drop de linhas inválidas em deals
deals_clean = deals[deals.dealsDateCreated < "2018-10-09"]
print(f'Número de linhas descartadas em deals:'
      f' {len(deals) - len(deals_clean)}')

# drop de linhas inválidas em contacts
contacts_clean = contacts[contacts.contactsDateCreated < "2018-10-09"]
print(f'Número de linhas descartadas em contacts:'
      f' {len(contacts) - len(contacts_clean)}')

# drop de linhas inválidas em companies
companies_clean = companies[
    companies.companiesDateCreated < "2018-10-09"]
print(f'Número de linhas descartadas em companies:'
      f' {len(companies) - len(companies_clean)}')

# # OUTPUT 01: Gráficos
# O primeiro OUTPUT deve conseguir servir de base para 2 gráficos:
# número de vendas por
...
n_vendas_contato = deals_contacts.groupby('contactsName')['dealsPrice'] \
...
fig.savefig(f'../reports/Valor Total Vendido por Contato.jpg')
...
for ano in range(2017, 2019):
    # print(ano)
    query_year = deals_clean[
        (deals_clean['dealsDateCreated'].dt.year == ano)]
    ...
    # visualização do gráfico
    n_vendas_mes_grafico = n_vendas_mes.plot(kind='bar', color='blue',
                                             label='Valor Vendido')
    plt.xlabel('Meses')
    ...
    fig.savefig(f'../reports/Valor_total_vendido_por_mês-{ano}.jpg')
    print(f"Output (Valor_total_vendido_por_mês-{ano}) salvo.")

# # OUTPUT 02: Lista das vendas por setores ordenado pelo mês
# O segundo output deve ser uma lista dos setores de empresa,
# ordenado por quanto esse setor representa no total
# vendido pela empresa no mês.
# Por exemplo, considerando que a empresa vendeu 10k total,
# se o 8k foi vendido para empresas do setor 1 e 2k
# para empresas do setor 2 então a lista resultante seria:
...
companies_sectors = pd.merge(
    companies_clean[colunas_necessarias_companies], sectors,
...
deals_companies_sectors = pd.merge(deals_clean, companies_sectors,
                                   left_on='companiesId',
                                   right_on='companiesId')

# Deixar a tabela já sorted por data
deals_companies_sectors.sort_values(by=['dealsDateCreated'],
                                    ascending=False)

# Neste dataframe vou armazenar
# o valor total vendido por mês, separado por setor
...
table_result['Valor Total Vendido Mensal'] = table_result[
    'Valor Total Vendido Mensal'].astype(int)

# percorre ano
for ano in range(2017, 2019):
    # print(ano)
    query_year = deals_companies_sectors[
        (deals_companies_sectors['dealsDateCreated'].dt.year == ano)]

    # percorre mês
    for mes in range(1, 13):
        # print(mes)
        query_year_month = query_year[
            (query_year['dealsDateCreated'].dt.month == mes)]

        # percorre setor (é menos custoso comparar valores numéricos,
        # então query_year_month['sectorKey'])
        for setor in query_year_month['sectorKey']:
            # print(setor)
            query_sector_month = query_year_month[
                (query_year_month['sectorKey'] == setor)]

            # group by dealsPrice
            grouping = query_sector_month.groupby('sector')[
                'dealsPrice'].sum().sort_values(ascending=False)

            # variável p organizar a inserção
            # no dataframe grouping_sector_month
...
# mapeamento do nome do setor.
# Aqui a tabela é bem menor e não é necessário fazer comparações
...
table_result.to_csv('../data/output.csv',
                    index=False,
                    sep=',',
                    encoding='utf-8')
print(
    'OUTPUT 02: Lista das vendas por '
    'setores ordenado pelo mês foi salvo no arquivo: output.csv')
